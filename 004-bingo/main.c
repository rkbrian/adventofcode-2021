...
int main(void)
{
	...
	char *lines = NULL, *token, *toten;
	size_t len = 0, leng = 0;
	ssize_t reading;
	int num_call[256], i = 0, j, k, m, calls = 0, flag;
	...
	while ((reading = getline(&lines, &leng, fp)) != -1)
	{
		if ((i + 1) % 6 != 0)
		{
			toten = strtok(lines, " ");
			...
			for (m = 0; m < calls; m++)
			{
				for (k = 0; k < 5; k++)
				{
					...
					for (j = 0; j < 5; j++)
					{
						...
						{
							logic_board[k][j] = 1;
							if (logic_board[k][0]+logic_board[k][1]+logic_board[k][2]+logic_board[k][3]+logic_board[k][4] == 5
							    || logic_board[0][j]+logic_board[1][j]+logic_board[2][j]+logic_board[3][j]+logic_board[4][j] == 5)
							{
								win_num = num_call[m], flag = 1;
								break;
							}
						}
					}
